...
class TenderAgent:
    ...
    async def run_tender_audit(
        ...
        # Policy & Risk Evaluation Gate
        action_req = await governed_agent.evaluate_and_gate(
            action_type="browse_read",
            description=f"Autonomous Procurement Audit: '{query[:60]}'",
            target=portal_target,
            payload={"query": query, "portal": portal_target, "headless": headless},
            telemetry_cb=self.telemetry_cb
        )
        ...
        if action_req.status == "REJECTED":
            await self.emit_telemetry(
                step="AGENT_HALTED",
                status="WARNING",
                message=f"Agent action blocked by policy or officer rejection: {action_req.decision_reason}"
            )
            return {
                "status": "HALTED_BY_POLICY",
                "reason": action_req.decision_reason,
                "tenders_found": 0,
                "report_markdown": "### 🛑 Action Halted\nThis operation was blocked or rejected by the Human-in-the-Loop policy gate."
            }

        audit_ledger.record_event(
            event_type="AGENT_ACTION_START",
            action=f"Browser agent launched for portal audit: {portal_target}",
            actor="AGENT",
            risk_level=action_req.risk_level,
            approval_status=action_req.status,
            details={"action_id": action_req.action_id, "query": query}
        )

        await self.emit_telemetry(
            step="INTENT_ANALYSIS",
            status="RUNNING",
            message=f"Analyzing intent: '{query}' -> Target: Government Procurement Portal (GeM/CPPP) [Risk: {action_req.risk_level}]"
        )
