...
class ProfileManager:
    ...
    def get_dictation_settings(self) -> Dict[str, Any]:
        """Returns the persisted dictation settings from the secure vault."""
        p = self._load_profile()
        default_dictation = {
            "enabled": True,
            "language": "en",
        ...
        user_dictation = p.get("dictation", {})
        default_dictation.update(user_dictation)
        return default_dictation

    def update_dictation_settings(self, enabled: Optional[bool] = None,
                                  language: Optional[str] = None,
                                  engine: Optional[str] = None,
                                  vad_enabled: Optional[bool] = None) -> Dict[str, Any]:
        """Persists updated dictation settings to profile.json in the local vault."""
        p = self._load_profile()
        if "dictation" not in p or not isinstance(p["dictation"], dict):
            p["dictation"] = {
                "enabled": True,
                "language": "en",
                "engine": "auto",
        ...
        if enabled is not None:
            p["dictation"]["enabled"] = bool(enabled)
        if language is not None:
            p["dictation"]["language"] = str(language).strip()
        if engine is not None:
            p["dictation"]["engine"] = str(engine).strip()
        if vad_enabled is not None:
            p["dictation"]["vad_enabled"] = bool(vad_enabled)
